...
<script>
    ...
    const getGroupId = () => $("meta[name=groupId]").attr("content");
...
`
    ...
    function sendMessage() {
        ...
        if (inputValue.length > 0 || hasFile) {
            ...
            formData.append("id", getMessengerId()); //group id
    ...
    channel.bind("client-typing", function(data) {
        if (data.group_id == getMessengerId()) {
            data.typing == true ?
    ...
    channel.bind("client-seen", function(data) {
        if (data.group_id == getMessengerId()) {
            if (data.seen == true) {
                ...
                console.log("[seen] event not triggered!");
            }
        }
    });
    ...
    channel.bind("client-contactItem", function(data) {
        if (data.update_for == auth_id) {
            data.updating == true ?
                ...
                console.log("[Contact Item updates] Updating failed!");
        }
    });
    ...
    /**
     *-------------------------------------------------------------
     * Check internet connection using pusher states //done
     *-------------------------------------------------------------
     */
    ...
    function setContactsLoading(loading = false) {
        if (!loading) {
            $(".listOfContactsGroup").find(".loading-contacts").remove();
        } else {
            $(".listOfContactsGroup").append(
        ...
        if (!contactsLoading && !noMoreContacts) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    if (contactsPage < 2) {
                        $(".listOfContactsGroup").html(data.contacts);
                    } else {
                        $(".listOfContactsGroup").append(data.contacts);
                    }
    ...
    /**
     *-------------------------------------------------------------
     * Update contact item //done
     *-------------------------------------------------------------
     */
    //done
    function updateContactItem(group_id) {
        let listItem = $("body")
            .find(".listOfContactsGroup")
        ...
        $.ajax({
            ...
            success: (data) => {
                const totalContacts =
                    $(".listOfContactsGroup").find(".messenger-list-item")?.length ||
                    0;
                if (totalContacts < 1)
                    $(".listOfContactsGroup").find(".message-hint").remove();
                listItem.remove();
                $(".listOfContactsGroup").prepend(data.contactItem);
                cssMediaQueries();
                updateSelectedContact(group_id);
            },
            error: () => {
    ...
    function deleteConversation(id) {
        $.ajax({
            ...
            success: (data) => {
                ...
                if (!data.deleted)
                    console.log("Error occurred, messages can not be deleted!");
    ...
    function deleteMessage(id) {
        $.ajax({
            ...
            success: (data) => {
                ...
                if (!data.deleted)
                    console.log("Error occurred, message can not be deleted!");
    ...
    $(document).ready(function() {
        ...
        $(".messenger-search").on("keyup", function(e) {
            $.trim($(this).val()).length > 0 ?
                ...
                $(".messenger-tab").hide() +
                $('.messenger-listView-tabs a[data-view="users"]').trigger(
                    "click"
                );
        });
        ...
        $("body").on("click", ".group-create-btn", function(e) {
            e.preventDefault();
            app_modal({
                show: true,
                name: "groupcreate",
        ...
        $("body").on("change", ".upload-avatar", (e) => {
            ...
            reader.addEventListener("load", (e) => {
                ...
                if (!file.type.match("image.*")) {
                    ...
                    console.log("File you selected is not an image!");
                } else {
                    // if the file is an image
                    $(".upload-avatar-preview").css(
                        "background-image",
    ...
    $(".audio-call").on("click", function() {
        callingRequest("audio_call");
    });

    $(".video-call").on("click", function() {
        callingRequest("video_call");
    });

    function callingRequest(type) {
        temporaryMsgId += 1;
        let tempID = `temp_${temporaryMsgId}`;

        const formData = new FormData($("#message-form")[0]);
        formData.append("id", getMessengerId()); //group_id
        formData.append("type", getMessengerType());
        formData.append("temporaryMsgId", tempID);
        formData.append("_token", access_token);
        ...
        $.ajax({
            ...
            beforeSend: () => {
                // remove message hint
                $(".messages").find(".message-hint").remove();
                // append a temporary message card for audio and video design
                messagesContainer
                    .find(".messages")
            ...
            success: (data) => {
                ...
                if (data.error > 0) {
                    ...
                    // add the message card coming from the server before the temp-card here i send the audo card
                    tempMsgCardElement.before(data.message);
                    // then, remove the temporary message card
                    tempMsgCardElement.remove();
                    // scroll to bottom
